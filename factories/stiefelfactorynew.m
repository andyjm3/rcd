function M = stiefelfactorynew(n, p, k, gpuflag)
...
%     end
    ...
    % inefficient matrix exponential retraction (for n^2 x n^2 matrix)
    M.retr_exp = @retr_exp;
    function Y = retr_exp(X, U, t)
        temp = U*X' - X*U' +X*(U'*X)*X';
        temp = t .* temp;
        Y = expm(temp) * X;
    end
    ...
    % Caley transform （efficient)
    %%% may require further projecting in the end
    M.retr_cl_light = @retr_cl_light;
    function Y = retr_cl_light(X, U, t)
        xtu = X'*U;
        Wu = (U - 0.5.*(X*xtu)) * X';
        Wu = Wu - Wu';
        ...
        Utemp = Wu * X;
        Y = X + t.* Utemp;
        for it = 1:cayley_iter
            Y = X + t/2 .* Wu * (X + Y);
        end
    end

    % Exponential map based on inversion of nxn matrix
    M.exp_large = @exp_large;
    function Y = exp_large(X, U, t)
        UXt = U * X';
        skewUXt = UXt - UXt';
        Y = expm(t.*skewUXt) * X;
    end
