function [X, infos] = pencf(problem, X0, options)
    [n p] = size(X0);
    ...
    for it = 1:maxiter
        ...
        dir = egradst - X *GtXsym + X*penalFeaX; % 2np^2 + p^2 + np

        
        if strcmp(options.stepsize_type, 'BB') % total: 6np
            % stepsize (BB1)
            if it == 1 
                stepsize = 1/( norm(X,'fro')/2*norm(dir,'fro') + p * options.lam);
            else
                Sk = X-Xprev;
        ...
        X = X - stepsize * dir; % 2np

        % https://gitee.com/stmopt/stop/blob/master/PenCF/stop_pencf.m
        X = X * min(1.0001*(sqrt(p)/norm(X,'fro')), 1); % 2np

        % total flops: BB: 5np^2 + 5p^2 + 11np
        % total flops: fix: 5np^2 + 5p^2 + 5 np
        ...
        fprintf('%s:  %3d\t%+.3e \t%.3e\n', 'PenCF', it, infos.optgap(it), stepsize);
    ...
    BBflops = 5*n*(p^2) + 5*(p^2) + 11*n*p;
    fixflops = 5*n*(p^2) + 5*(p^2) + 5*n*p;
